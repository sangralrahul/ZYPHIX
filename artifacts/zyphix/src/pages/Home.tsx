              ...
              {submitted ? (
                ...
                <motion.div initial={{ scale:.8, opacity:0 }} animate={{ scale:1, opacity:1 }} transition={{ type:'spring', stiffness:200 }}
                  ...
                  <motion.div initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }} transition={{ delay:.45 }}
                    ...
                    {PARTNER_ROLES.includes(form.role) ? (
                      ...
                      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                        {[
                          ...
                          <motion.div key={i} initial={{ opacity:0, x:-10 }} animate={{ opacity:1, x:0 }} transition={{ delay: .5 + i * .1 }}
                            style={{ display:'flex', gap:12, alignItems:'flex-start', padding:'10px 12px', borderRadius:12, background:'#F8FAFB', border:`1px solid ${BD}` }}>
                      ...
                      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                        {[
                          ...
                          <motion.div key={i} initial={{ opacity:0, x:-10 }} animate={{ opacity:1, x:0 }} transition={{ delay: .5 + i * .1 }}>
                            <a href={href ?? '#'} style={{ textDecoration:'none', display:'flex', gap:12, alignItems:'flex-start', padding:'10px 12px', borderRadius:12, background:'#F8FAFB', border:`1px solid ${BD}`, transition:'border-color .15s, background .15s' }}
                              onMouseEnter={e=>{ (e.currentTarget as HTMLAnchorElement).style.background=`${G}08`; (e.currentTarget as HTMLAnchorElement).style.borderColor=`${G}50`; }}
                              onMouseLeave={e=>{ (e.currentTarget as HTMLAnchorElement).style.background='#F8FAFB'; (e.currentTarget as HTMLAnchorElement).style.borderColor=BD; }}>
                              <span style={{ fontSize:20, flexShrink:0, lineHeight:1 }}>{icon}</span>
                              <div>
                                <p style={{ fontSize:13, fontWeight:700, color:T1, marginBottom:2 }}>{title}</p>
                                <p style={{ fontSize:12, color:T2, lineHeight:1.5 }}>{sub}</p>
                    ...
                    {/* WhatsApp CTA */}
                    <motion.a href="https://wa.me/91XXXXXXXXXX?text=Hi%20Zyphix%2C%20I%20just%20signed%20up%20and%20have%20a%20question."
                      target="_blank" rel="noopener noreferrer"
                      initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:.85 }}
                      style={{ marginTop:16, display:'flex', alignItems:'center', justifyContent:'center', gap:8, padding:'11px', borderRadius:12, background:'#F0FDF4', border:`1px solid ${G}40`, textDecoration:'none', color:G, fontSize:13, fontWeight:700, transition:'background .15s' }}
                      onMouseEnter={e=>{ (e.currentTarget as HTMLAnchorElement).style.background=`${G}15`; }}
