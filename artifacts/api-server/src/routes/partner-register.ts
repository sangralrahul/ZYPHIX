...
async function generatePDF(data: {
  ...
  page.drawText("India's SuperLocal App", {
    x: 104,
    y: height - 76,
    size: 9,
    font: fontReg,
    color: rgb(0.43, 0.91, 0.6),
  });
  ...
  /* ── Title ── */
  page.drawText("Partner Registration", {
    x: 50,
    y: height - 148,
    size: 18,
    font: fontBold,
  ...
  // Divider line
  page.drawLine({
    start: { x: 50, y: height - 178 },
    end: { x: width - 50, y: height - 178 },
    thickness: 1,
    color: LGRAY,
  });
...
router.post("/partner-register", async (req, res) => {
  ...
  const missing = (["name", "email", "phone", "city", "role"] as const).filter(
  ...
  <div style="margin-top:20px;">
    ...
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="border-radius:12px;overflow:hidden;border:1px solid #E5E7EB;">
      ${Object.entries(details)
        .filter(([, v]) => (Array.isArray(v) ? (v as string[]).length > 0 : String(v || "").trim()))
        .map(([key, val], i) => {
...
<tr><td style="background:#F9FAFB;padding:16px 40px;text-align:center;">
  <p style="margin:0;font-size:11px;color:#9CA3AF;">© 2025 Zyphix · Jammu, J&K · <a href="mailto:partner@zyphix.in" style="color:#0DA366;text-decoration:none;">partner@zyphix.in</a></p>
    ...
    res.json({
      success: true,
      message: "Registration submitted! You'll receive a confirmation email shortly.",
    });
  } catch (err: unknown) {
